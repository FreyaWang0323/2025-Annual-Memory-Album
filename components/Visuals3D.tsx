...
import { Effects, Edges, useTexture, Stars, Sparkles } from '@react-three/drei';
...
const SceneContent: React.FC<Visuals3DProps> = ({ items, mode, scrollOffset, focusedId, frameScale, nebulaDensity, driftSpeed, browseSpacing, focusZoom }) => {
  ...
  useEffect(() => {
    scene.fog = new THREE.FogExp2('#2a0404', 0.05);
  ...
  return (
    <>
      ...
      <EffectComposer enableNormalPass={false}>
        {/* Adjusted Bloom to only affect very bright elements (edges), leaving photos mostly natural */}
        <Bloom 
            ...
            radius={0.4}
        />
        {/* Removed Noise and Vignette as requested for cleaner photos */}
      </EffectComposer>
    </>
  );
};
...
export const Visuals3D: React.FC<Visuals3DProps> = (props) => {
  return (
    <Canvas 
        ...
        dpr={[1, 2]} 
        gl={{ 
            antialias: true, // Enable AA for cleaner edges since we removed noise/vignette masking
            alpha: false,
